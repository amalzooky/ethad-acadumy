  ...
  if (news) {
    for (var n = 0; n < news.length; n++) {
      ...
      var title = news[n].title,
        id = news[n].id,
        d = new Date(Date.parse(news[n].created_at)).toDateString(),
        img = "<img src='" + news[n].image + "' />"
      a += "<li>" + img + '<h3><a href="' + "/dashboard/news/" + id + '">' + title + '</a></h3><div class="tickermeta">' + d + "</div></li>"
    }
    a += "</ul>", e.innerHTML = a
  }
  $("#ticker").vTicker()
