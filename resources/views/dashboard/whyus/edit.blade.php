...
@section('title') @lang('dashboard.lecture.edit') @stop
            ...
            <div class="col">
                <div class="card">
                  ...
                  <div class="card-header">
                    <h3 class="card-title">@lang('dashboard.lecture.edit')</h3>
                  ...
                  <div class="card-body">
                    <form action="{{route('lectures.update', $lecture->id)}}" method="POST" class="ajax">
                        @method('patch')
                        @csrf
                        <div class="form-group">
                            ...
                            <div class="input-icon">  
                                ...
                                <input type="text" class="form-control" id="name" name="name" placeholder="@lang('dashboard.lecture.name')"  value="{{$lecture->name}}" required autofocus>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="description" class="form-label">@lang('dashboard.lecture.description')</label>
                            <textarea class="form-control" id="description" name="description" rows="5" placeholder="@lang('dashboard.lecture.description')..." required>{{$lecture->description}}</textarea>
                        </div>
                        <div class="row">
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control custom-select lecture_subject" name="subject_id" required>
                                        ...
                                        <option value="{{$subject->id}}" {{$lecture->subject_id === $subject->id ? 'selected' : ''}}>{{$subject->name}} / {{$subject->major->name}} / {{$subject->semester}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <label class="custom-switch">
                                <input type="checkbox" name="is_active" id="is_active" class="custom-switch-input" value="1" {{$lecture->is_active ? 'checked' : ''}}>
